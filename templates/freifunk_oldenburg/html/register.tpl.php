...
<form action="./register.php{if isset($smarty.get.openid)}?openid={$smarty.get.openid}{/if}" method="POST">
  {if !empty($smarty.get.openid)}<p>Open-ID:<br><input name="openid" type="text" size="30" maxlength="30" value="{$smarty.get.openid}"> <a href="./register.php"><img src="./templates/{$template}/img/arrow_undo.png" style="border-style: none;"></a></p>{/if}
  <p>Benutzername:<br><input name="nickname" type="text" size="30" maxlength="30" value="{$smarty.post.nickname}"> {if empty($smarty.get.openid)}<a href="./register.php?openid=openid.example.com"><img src="./templates/{$template}/img/openid-logo-small.png" style="border-style: none;"></a>{/if}</p>
